          ...
          <div class="col-md-12">
            <div class="card">
              <div class="card-heading">
                <h4 class="h4 text-center p-3 bg-info">Publish Ads & Advertisement Setting</h4>
              </div>
              <div class="card-body">
              <!--  -->
              <div class="form-row">
                ...
                <div class="table-responsive">
                  <table class="table table-striped table-sm table-hover table-bordered table-condenced" id="table_">
                   ...
                   <tr>
                    ...
                    <td><?=$row['pkg_name'];?></td>
                    <td><?php echo get_vendor_name($row['vendor_id'],$conn);?></td>
                    <td><?php echo $row['descriptions'];?></td>
                    <td><?=$row['created_at'];?></td>
                    <td><?=$row['expiry_date'];?></td>
                    <td><?=$row['publish_status'];?></td>
                    <td>
                    <button class="btn btn-info btn-sm"  data-toggle="modal" data-target="#addStaff_<?=$row['request_id'];?>"><i class="fa fa-edit"></i></button>
<!--                    
                    <button class="btn btn-danger btn-sm" value=""  onclick="delete_member_data(this.value);"><i class="fa fa-trash"></i></button> -->
                    
                    </td>
                    <div class="modal fade" id="addStaff_<?=$row['request_id'];?>" role="dialog">  
                  <div class="modal-dialog modal-lg">  
                    <div class="modal-content">
                    <form action="action.php" method="POST" enctype="multipart/form-data">
                      <div class="modal-header">  
                        <h4 class="modal-title">Advertisement Setting</h4>  
                        <button type="button" class="close" data-dismiss="modal">×</button>  
                      </div>  
                      <div class="modal-body"> 
                      <div class="form-row">
                        <input type="hidden" name="ads_id_req" value="<?=$row['request_id'];?>">
                      ...
                      <div class="modal-footer">  
                        ...
                        <button type="submit" name="update_status_ads_publish" class="btn btn-success btn-sm "> Update</button>
  ...
  <div class="modal fade" id="editmember" role="dialog">  
    <div class="modal-dialog modal-lg">  
      <div class="modal-content">  
        <div class="modal-header">  
          <h4 class="modal-title">Edit Ads information</h4>  
          <button type="button" class="close" data-dismiss="modal">×</button>  
        </div>  
  ...
  <div class="modal fade" id="addStaff" role="dialog">  
    <div class="modal-dialog modal-lg">  
      ...
      <form action="action.php" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-row">
            <div class="form-group col-md-12">
                <label for="name">Select Vendor Name</label>
                <select name="ads_id_req" class="form-control">
                  <option value="">Choose Vendor Name</option>
                  <?php 
                   $userdata=$conn->query("SELECT tbl_ads_requests.*,tbl_products.product_name,tbl_advertisment_packages.pkg_name FROM `tbl_ads_requests` INNER JOIN tbl_products ON tbl_products.product_id=tbl_ads_requests.product_id INNER JOIN tbl_advertisment_packages ON tbl_advertisment_packages.pkg_id=tbl_ads_requests.pkg_id where tbl_ads_requests.ads_status='accept'");
                   $count=0;
                   while($row=$userdata->fetch_assoc()):
                
                ?>
                <option value="<?=$row['request_id']?>">
                <?php echo get_vendor_name($row['vendor_id'],$conn).' ('.$row['pkg_name'].')';?></option>
                <?php
                endwhile;
                ?>
                </select>
            ...
            <div class="form-group col-md-12">
                <label for="name">Advertising image (470 x 620)</label>
               <input type="file" name="advsrting_image" class="form-control">
            </div>
            <div class="form-group col-md-12">
        ...
        <div class="modal-footer">  
          <button type="button" class="btn btn-info btn-sm" data-dismiss="modal">Close</button>
          <button type="submit" name="adve_adverting_Ads" class="btn btn-success btn-sm "> Publish</button>
...
$(function(){
    ...
    $("#table_").DataTable({
        ...
        // Array(doc.content[1].table.body[0].length + 1).join('*').split('');
            ...
            var Customer_name="ZoneElectronics";
